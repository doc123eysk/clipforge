                  ...
                  {clip.status === "ready" && (
                    <div className="mt-1 flex flex-col gap-1">
                      ...
                      {published[clip.id]?.video ? (
                        <a href={published[clip.id].video} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="flex items-center justify-center gap-1 rounded-lg bg-green-500/20 py-1 text-[10px] text-green-300 transition hover:bg-green-500/30">
                          Видео ✓
                        </a>
                      ) : (
                        <button
            ...
            <div className="absolute -bottom-12 left-0 right-0 flex items-center justify-between text-sm text-zinc-400">
              ...
              <div className="flex gap-2">
                {published[previewClip.id]?.video ? (
                  <a href={published[previewClip.id].video} target="_blank" rel="noopener noreferrer" className="rounded-lg bg-green-500/20 px-3 py-1.5 text-xs text-green-300 transition hover:bg-green-500/30">Видео ✓</a>
                ) : (
                  <button onClick={(e) => publishToVK(previewClip.id, "video", e)} disabled={publishing === `${previewClip.id}-video`} className="rounded-lg bg-[#4C75A3]/20 px-3 py-1.5 text-xs text-[#6d9fd4] transition hover:bg-[#4C75A3]/30 disabled:opacity-50">
                    {publishing === `${previewClip.id}-video` ? "..." : "Видео"}
                  </button>
                )}
                {published[previewClip.id]?.clip ? (
                  <span className="rounded-lg bg-green-500/20 px-3 py-1.5 text-xs text-green-300">Клип ✓</span>
                ) : (
                  <button onClick={(e) => publishToVK(previewClip.id, "clip", e)} disabled={publishing === `${previewClip.id}-clip`} className="rounded-lg bg-pink-500/20 px-3 py-1.5 text-xs text-pink-300 transition hover:bg-pink-500/30 disabled:opacity-50">
